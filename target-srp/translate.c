...
static void disas_srp_insn(CPUState * env, DisasContext *s) {
	...
	case 3:
		...
		case 0x1E:
			...
			rs = (insn >> 8) & 0xff;
			//#ifdef USE_SYSTEMC
			//	tcg_gen_movi_i32(cpu_R[rd], cpu_socket_read(rs, num_socket++));
			//#else
			addr = load_reg(s, rs);
		...
		case 0x3E:
			...
			if (USE_SOCKET) {
				tmp1 = new_tmp();
				gen_helper_cpu_socket_read(tmp1, rs);
				store_reg(s, rd, tmp1);
				dead_tmp(tmp1);
			} else {
				addr = new_tmp();
		...
		case 0x3F:
			...
			if (USE_SOCKET) {
				tmp1 = load_reg(s, rs);
				gen_helper_cpu_socket_write(rd, tmp1);
				dead_tmp(tmp1);
			} else {
				addr = new_tmp();
				tcg_gen_movi_i32(addr, rd);
	...
	case 6:
		...
		case 0x4E:
			...
			if (USE_SOCKET) {
				tmp1 = new_tmp();
				gen_helper_cpu_socket_read(tmp1, imm32);
				store_reg(s, rd, tmp1);
				dead_tmp(tmp1);
			} else {
				addr = new_tmp();
				tcg_gen_movi_i32(addr, imm32);
				tmp1 = gen_ld32(addr, IS_USER(s));
				store_reg(s, rd, tmp1);
				dead_tmp(addr);
			}
		...
		case 0x4F:
			...
			if (USE_SOCKET) {
				tmp1 = load_reg(s, rs);
				gen_helper_cpu_socket_write(imm32, tmp1);
				dead_tmp(tmp1);
			} else {
				addr = new_tmp();
				tcg_gen_movi_i32(addr, imm32);
...
void cpu_dump_state(CPUState *env, FILE *f, int(*cpu_fprintf)(FILE *f,
	...
	uint32_t psr;
	psr = psw_read(env);
	cpu_fprintf(f, "PSR=%08x %c%c%c%c\t  ", psr, psr & (1 << 15) ? 'C' : '-',
			psr & (1 << 14) ? 'Z' : '-', psr & (1 << 13) ? 'S' : '-', psr & (1
					<< 12) ? 'O' : '-');
